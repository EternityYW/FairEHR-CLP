...
# Load data
df_structured = pd.read_csv('structured_Stanford.csv')
df_notes = pd.read_csv('unstructured_Stanford.csv') 

df_demographics = df_structured[['pat_deid', 'sex', 'ethnic_group', 'race', 'age', 'surg_family', 'product_type', 'tobacco_user']]
df_demographics = df_demographics.drop_duplicates()
df_longitudinal = df_structured[['Heart Rate', 'Pulse', 'Resp', 'SpO2','Temp', 'Systolic_BP', 'Diastolic_BP', 'ALT (SGPT), Ser/Plas', 'Albumin, Ser/Plas', 'Anion Gap', 'BUN, Ser/Plas', 'CO2, Ser/Plas', 'Calcium, Ser/Plas', 'Chloride, Ser/Plas', 'Creatinine, Ser/Plas', 'Glucose, Ser/Plas', 'Hematocrit', 'Hemoglobin', 'MCH', 'MCHC', 'MCV', 'Platelet count', 'Potassium, Ser/Plas', 'RBC', 'RDW', 'Sodium, Ser/Plas', 'WBC']]
df_labels = df_structured[['pat_deid', 'readmission_30_days_label']]
...
longitudinal_data = df_longitudinal.values.reshape((46933, 12, 27))
...
# class_weights = torch.tensor([2.8, 1.0], dtype=torch.float) (DELIRIUM)
# class_weights = torch.tensor([1.8, 1.0], dtype=torch.float) (OUD)
class_weights = torch.tensor([1.5, 1.0], dtype=torch.float)
