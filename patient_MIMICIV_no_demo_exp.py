...
# Load data
df_structured = pd.read_csv('structured_MIMICIV.csv')
...
df_longitudinal = df_structured[['heart_rate', 'sbp', 'dbp', 'mbp', 'resp_rate', 'temperature', 'spo2',
...
longitudinal_data = df_longitudinal.values.reshape((15918, 12, 27))
...
#class_weights = torch.tensor([2.0, 1.0], dtype=torch.float) (OUD)

class_weights = torch.tensor([3.5, 0.85], dtype=torch.float)
