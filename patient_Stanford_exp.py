...
synthetic_sex = df_demographics['sex'].apply(lambda x: generate_different_category(x, [0, 1]))
synthetic_ethnic_group = df_demographics['ethnic_group'].apply(lambda x: generate_different_category(x, [0, 1, 2]))
synthetic_race = df_demographics['race'].apply(lambda x: generate_different_category(x, [0, 1, 2, 3, 4, 5]))
synthetic_coverage = df_demographics['product_type'].apply(lambda x: generate_different_category(x, [0, 1, 2, 3]))
...
df_synthetic_demographics = pd.DataFrame({
    ...
    'race': synthetic_race,
    'age': synthetic_age,
    'surg_family': df_demographics['surg_family'],
    'product_type': synthetic_coverage,
    'tobacco_user': df_demographics['tobacco_user']
})

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

longitudinal_features = df.loc[:, 'Heart Rate':'WBC']
...
df_real_demographics = df_demographics[['sex', 'ethnic_group', 'race', 'age', 'surg_family', 'product_type', 'tobacco_user']]
...
class FairnessAwareModel(nn.Module):
    def __init__(self):
        ...
        self.demographics_branch = nn.Sequential(nn.Linear(7, 32), nn.BatchNorm1d(32), nn.ReLU())
...
# class_weights = torch.tensor([0.25, 1.0], dtype=torch.float) (DELIRIUM)
# class_weights = torch.tensor([0.7, 1.0], dtype=torch.float) (OUD)
class_weights = torch.tensor([0.8, 1.0], dtype=torch.float) 
